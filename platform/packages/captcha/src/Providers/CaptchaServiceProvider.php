...
class CaptchaServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        PanelSectionManager::default()->beforeRendering(function () {
            PanelSectionManager::registerItem(
                ...
                fn () => PanelSectionItem::make('captcha')
                    ->setTitle(trans('packages/captcha::captcha.settings.title'))
                    ->withIcon('ti ti-refresh')
                    ->withPriority(150)
                    ->withDescription(trans('packages/captcha::captcha.settings.panel_description'))
                    ->withRoute('captcha.settings')
            );
        });

        CaptchaFacade::registerFormSupport(LoginForm::class, LoginRequest::class, trans('packages/captcha::captcha.admin_login_form'));
        CaptchaFacade::registerFormSupport(ForgotPasswordForm::class, ForgotPasswordRequest::class, trans('packages/captcha::captcha.admin_forgot_password_form'));
        CaptchaFacade::registerFormSupport(ResetPasswordForm::class, ResetPasswordRequest::class, trans('packages/captcha::captcha.admin_reset_password_form'));
        ...
        add_filter('form_extra_fields_render', function (?string $fields = null, ?string $form = null): ?string {
            ...
            return $fields . view('packages/captcha::forms.old-version-support', compact('form'))->render();
        }, 128, 2);

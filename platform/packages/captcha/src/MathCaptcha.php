    ...
    protected function getMathOperand(): string
    {
        if (! $this->session->get('math-captcha.operand')) {
            $this->session->put(
                ...
                config(
                    'packages.captcha.general.math-captcha.operands.' . array_rand(
                        config('packages.captcha.general.math-captcha.operands')
                    )
                )
            );
        }
    ...
    protected function getMathFirstOperator(): int
    {
        if (! $this->session->get('math-captcha.first')) {
            $this->session->put(
                'math-captcha.first',
                rand(
                    config('packages.captcha.general.math-captcha.rand-min'),
                    config('packages.captcha.general.math-captcha.rand-max')
                )
            );
        }
